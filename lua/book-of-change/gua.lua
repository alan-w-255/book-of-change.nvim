local gua = {
    {
        "第一卦 乾 乾为天 乾上乾下",
        {
            "乾：元，亨，利，贞。彖曰：大哉乾元，万物资始，乃统天。云行雨施，品物流形。大明始终，六位时成，时乘六龙以御天。乾道变化，各正性命，保合大和，乃利贞。首出庶物，万国咸宁。象曰：天行健，君子以自强不息。"
        },
...
setmetatable(gua, {
    __index = function(t, k)
        if type(k) == "number" then return t[k] end
        if type(k) == "string" then return t[guaxiang[k]] end
        return nil
    end
})
